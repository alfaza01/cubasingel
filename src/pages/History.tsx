...
export function History() {
  ...
  const handleExportTransactionsToExcel = async () => {
    try {
      ...
      const cleanSearch = escapeHtml(searchQuery || '-');
      const dateRangeStr = escapeHtml(`${startDate || 'Semua'} s.d. ${endDate || 'Semua'}`);
      const printDateStr = escapeHtml(new Date().toLocaleString('id-ID'));

      const endRow = 11 + filteredTransactions.length;
  ...
  <table>
    ...
    <thead>
      ...
      <tr>
        ...
        <td class="summary-value text-center font-bold" colspan="2">\${filteredTransactions.length} Trx</td>
...
`;
      ...
      if (isAndroid) {
        ...
        await Share.share({
          title: 'Riwayat Transaksi Excel',
          text: 'Laporan Riwayat Transaksi Toko',
          url: savedFile.uri,
