...
import { 
  ...
  Edit3, X, Eye, EyeOff, History, BarChart3, QrCode, Coins, PlusCircle, ClipboardCheck, Scan, Camera, FileDown
} from 'lucide-react'
...
export const VoucherView: React.FC<VoucherViewProps> = (props) => {
  ...
  const saveOnlineProducts = async (updatedList: VoucherItem[]) => {
    ...
    try {
      const promises = updatedList.map(item => { return Promise.resolve(); });
      ...
      try {
        ...
        if (data.products && Array.isArray(data.products) && data.products.length > 0) {
          ...
          data.products.forEach((p: any) => {
            ...
            // PROVIDER_CATEGORIES: TELKOMSEL, XL, AXIS, INDOSAT, TRI, SMARTFREN, PULSA, E-TOLL, LAINNYA
          ...
          props.showToast(`Berhasil ekstrak ${data.products.length} produk! Silahkan cek di Mode Massal.`);
          setTambahMode('massal');
        } else {
          props.showToast('Gagal memproses gambar. Tidak ada produk terdeteksi.');
        }
      } catch (err: any) {
  ...
  const handleExportToExcel = () => {
    try {
      if (filteredItems.length === 0) {
        props.showToast('TIDAK ADA DATA UNTUK DIEKSPOR!')
        return
      }
...
`;
      ...
      props.showToast('EKSPOR EXCEL BERHASIL DIUNDUH!')
    } catch (err: any) {
      console.error(err)
      props.showToast('GAGAL MENGEKSPOR EXCEL: ' + err.message)
    }
  }
      ...
      <div className="flex flex-col pb-14">
          ...
          <div className="w-full max-w-5xl mx-auto flex flex-col mb-12">
            ...
            <div className="flex items-center gap-2 mb-4 shrink-0">
              ...
              <button
                ...
                className="bg-emerald-600 hover:bg-emerald-700 text-white p-3 rounded-2xl flex items-center justify-center gap-1.5 text-xs font-black uppercase tracking-wider transition-all shadow-md active:scale-95 cursor-pointer shrink-0"
                title="Ekspor ke MS Excel"
              >
                ...
                {filteredItems.length === 0 ? (
                          ...
                          <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                            ...
                            {props.kasirRole === 'owner' && (
                              <button
                                ...
                                className="mt-3 w-full py-2.5 bg-indigo-50 hover:bg-indigo-600 dark:bg-indigo-950/25 text-indigo-700 hover:text-white dark:text-indigo-400 dark:hover:bg-indigo-700 border border-indigo-200 dark:border-indigo-800 rounded-xl font-black text-[9px] uppercase tracking-wider flex items-center justify-center gap-1.5 cursor-pointer transition-all active:scale-95"
                              >
                                <Edit3 size={11} className="stroke-[2.5]" />
                                <span>Klik Edit Detail</span>
                              </button>
                            )}
                          </div>
                  ...
                  <div className="p-1 sm:p-3">
                    ...
                    <div className="overflow-x-auto w-full border border-slate-150 rounded-2xl bg-white dark:bg-slate-800 shadow-3xs">
                      <table className="w-full border-collapse text-left select-none text-[10.5px] sm:text-[11px]">
                        <thead>
                          <tr className="bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400 font-black tracking-wider uppercase text-[8px] sm:text-[8.5px] border-b border-slate-150 whitespace-nowrap">
                            <th className="p-2.5 sm:p-3 border-r border-slate-200 dark:border-slate-700 text-center select-none font-mono tracking-normal whitespace-nowrap w-[1%]">No</th>
                            <th className="p-2.5 sm:p-3 border-r border-slate-200 dark:border-slate-700 whitespace-nowrap w-auto">Nama Produk Voucher</th>
                            <th className="p-2.5 sm:p-3 border-r border-slate-200 dark:border-slate-700 text-center whitespace-nowrap w-[1%]">Provider</th>
                            <th className="p-2.5 sm:p-3 border-r border-slate-200 dark:border-slate-700 text-right whitespace-nowrap w-[1%]">Modal</th>
                            <th className="p-2.5 sm:p-3 border-r border-slate-200 dark:border-slate-700 text-right whitespace-nowrap w-[1%]">Harga Jual</th>
                            <th className="p-2.5 sm:p-3 border-r border-slate-200 dark:border-slate-700 text-right text-emerald-700 font-black whitespace-nowrap w-[1%]">Profit</th>
                            <th className="p-2.5 sm:p-3 border-r border-slate-200 dark:border-slate-700 text-center text-slate-500 dark:text-slate-400 font-bold whitespace-nowrap w-[1%]">Awal</th>
                            <th className="p-2.5 sm:p-3 border-r border-slate-200 dark:border-slate-700 text-center text-indigo-500 font-bold whitespace-nowrap w-[1%]">QRIS</th>
                            <th className={cn("p-2.5 sm:p-3 text-center text-emerald-600 font-black font-extrabold whitespace-nowrap w-[1%]", props.kasirRole === 'owner' ? "border-r border-slate-250 dark:border-slate-700" : "")}>Stok Akhir</th>
                            {props.kasirRole === 'owner' && (
                              <th className="p-2.5 sm:p-3 text-center font-black whitespace-nowrap w-[1%]">Aksi (CLOUD)</th>
                            )}
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 bg-white dark:bg-slate-800 whitespace-nowrap">
                          {filteredItems.map((item, idx) => {
                            ...
                            return (
                              <tr key={item.id} className="hover:bg-indigo-50/10 bg-white dark:bg-slate-800 leading-normal font-bold">
                                <td className="p-2.5 sm:p-3 border-r border-slate-150 text-center text-slate-400 font-mono font-bold select-none whitespace-nowrap w-[1%]">{idx + 1}</td>
                                <td className="p-2.5 sm:p-3 border-r border-slate-150 uppercase tracking-wide text-slate-800 dark:text-slate-100 text-[11px] sm:text-[12px] whitespace-nowrap select-text w-auto">{item.name}</td>
                                <td className="p-2.5 sm:p-3 border-r border-slate-150 text-center whitespace-nowrap w-[1%]">
                                  <span className={cn("text-[7px] font-black tracking-widest border px-1.5 py-0.5 rounded uppercase block w-fit mx-auto whitespace-nowrap", getBadgeStyles(item.brand))}>
                                    {item.brand}
                                  </span>
                                </td>
                                <td className="p-2.5 sm:p-3 border-r border-slate-150 text-right font-mono text-slate-500 dark:text-slate-400 whitespace-nowrap w-[1%]">
                                  {props.kasirRole === 'owner' ? formatRupiah(item.costPrice) : '🔒 [OWNER]'}
                                </td>
                                <td className="p-2.5 sm:p-3 border-r border-slate-150 text-right font-mono text-indigo-600 font-extrabold whitespace-nowrap w-[1%]">{formatRupiah(item.price)}</td>
                                <td className="p-2.5 sm:p-3 border-r border-slate-150 text-right font-mono text-emerald-600 font-extrabold whitespace-nowrap w-[1%]">
                                  {props.kasirRole === 'owner' ? `+${formatRupiah(isProfit)}` : '🔒'}
                                </td>
                                <td className="p-2.5 sm:p-3 border-r border-slate-150 text-center font-mono text-slate-500 dark:text-slate-400 font-bold select-none whitespace-nowrap w-[1%]">{getProductAwalStock(item)}</td>
                                <td className="p-2.5 sm:p-3 border-r border-slate-150 text-center font-mono text-indigo-600 font-extrabold select-none whitespace-nowrap w-[1%]">{(nontunaiSales[item.id] || 0)}</td>
                                <td className={cn("p-2.5 sm:p-3 text-center font-mono select-none whitespace-nowrap w-[1%]", props.kasirRole === 'owner' && "border-r border-slate-150")}>
                                  <span className={cn(
                                    "px-3 py-1 rounded-lg text-[10px] sm:text-[10.5px] font-black whitespace-nowrap inline-flex items-center justify-center gap-1", 
                                ...
                                {props.kasirRole === 'owner' && (
                                  <td className="p-2.5 sm:p-3 text-center select-none font-sans">
                                    <button
                                      onClick={() => {
                                        handleStartEdit(item);
                                        setActiveTab('master');
                                      }}
                                      className="mx-auto px-3 py-1.5 bg-indigo-50 hover:bg-indigo-600 dark:bg-indigo-950/20 text-indigo-700 hover:text-white dark:text-indigo-400 dark:hover:bg-indigo-700 border border-indigo-200 dark:border-indigo-800 rounded-xl text-[9px] font-black uppercase flex items-center justify-center gap-1.5 cursor-pointer transition-all active:scale-95 whitespace-nowrap"
                                    >
                                      <Edit3 size={11} className="stroke-[2.5]" />
                                      <span>EDIT</span>
                                    </button>
        ...
        {activeTab === 'master' && (
          <div className="w-full max-w-5xl mx-auto flex flex-col">
            ...
            {/* View Mode Switcher Header Control */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl mb-4 select-none shrink-0 font-sans">
              <div className="text-left">
                <p className="text-[8.5px] font-black text-indigo-750 dark:text-indigo-400 uppercase tracking-widest leading-none mb-1">
                  PILIH TAMPILAN MASTER PRODUK
                </p>
              ...
              {/* Toggles using responsive layouts and export button */}
              <div className="w-full sm:w-auto flex flex-wrap items-center gap-2">
                <button
                  type="button"
                  onClick={handleExportToExcel}
                  className="bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-2 rounded-xl flex items-center justify-center gap-1.5 text-[8.5px] xs:text-[9.5px] font-black uppercase tracking-wider transition-all shadow-md active:scale-95 cursor-pointer"
                  title="Ekspor ke MS Excel"
                >
                ...
                <div className="grid grid-cols-2 gap-1 bg-slate-200 dark:bg-slate-755 p-0.5 rounded-xl border border-slate-250 dark:border-slate-700 select-none min-w-[200px]">
                  <button
                    type="button"
            ...
            <div className="px-1 overflow-visible">
              ...
              {filteredItems.length === 0 ? (
                ...
                <>
                      ...
                      {filteredItems.map(item => {
                        ...
                        return (
                          <div
                            key={item.id}
                            className={cn(
                              "bg-white dark:bg-slate-800 p-5 rounded-[2rem] border transition-all text-left flex flex-col gap-3 shadow-xs relative font-sans",
                              isItemEditing
                                ? "border-indigo-400 ring-2 ring-indigo-55 dark:ring-indigo-950 shadow-sm"
                          ...
                          >
                            {isItemEditing ? (
                              <>
                                ...
                                {/* Actions bar for save or cancel */}
                                <div className="flex gap-2 pt-2 border-t border-slate-100 dark:border-slate-800 mt-2 select-none">
                                  <button
                                    onClick={() => handleSaveEditItem(item.id)}
                                    className="flex-1 font-black text-[9px] uppercase tracking-wider py-2.5 rounded-xl flex items-center justify-center gap-1 cursor-pointer transition-all border shadow-xs bg-emerald-600 border-emerald-700 hover:bg-emerald-500 text-white"
                                  >
                                    <Save size={11} />
                                  ...
                                  <button
                                    ...
                                    className="px-3.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-205 text-slate-700 dark:text-slate-350 font-black text-[9px] uppercase tracking-wider py-2.5 rounded-xl flex items-center justify-center gap-1.5 cursor-pointer transition-all border border-slate-250 dark:border-slate-700 shadow-xs"
                                  >
                                    <X size={12} />
                                    <span>Batal</span>
                                  </button>
                              ...
                              <>
                                ...
                                <div className="flex items-center justify-between gap-1 mb-1.5 pb-1 border-b border-slate-150 dark:border-slate-850 flex-wrap font-sans">
                                  ...
                                  <div className="flex gap-1.5 flex-wrap">
                                    <span className="text-[8px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-slate-50 dark:bg-slate-900 px-2 py-0.5 rounded border border-slate-150 dark:border-slate-700">
                                      MODAL: {formatRupiah(item.costPrice)}
                                    </span>
                                    <span className="text-[8px] font-black text-indigo-700 bg-indigo-50 border border-indigo-100 dark:text-indigo-400 dark:bg-slate-900/45 px-2 py-0.5 rounded uppercase tracking-wider">
                                      JUAL: {formatRupiah(item.price)}
                                    </span>
                                    <span className="text-[9px] font-bold text-slate-700 dark:text-slate-200 font-mono bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded leading-none">
                                      STOK: {item.stock} PCS
                                    </span>
                                  </div>
                                </div>
                                ...
                                <div className="p-3 bg-indigo-50/20 border border-indigo-150 dark:border-indigo-900/30 rounded-2xl my-2 flex flex-col gap-2">
                                  <div className="flex items-center justify-between">
                                    <span className="text-[8px] font-black text-indigo-750 dark:text-indigo-400 uppercase tracking-widest flex items-center gap-1.5">
                                      <QrCode size={12} className="text-indigo-500" />
                                      <span>LAKU NON-TUNAI (QRIS)</span>
                                    </span>
                                    <span className="text-xs font-black font-mono text-indigo-750 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950 border border-indigo-200 dark:border-indigo-900 px-2.5 py-0.5 rounded-lg select-all">
                                  ...
                                  <div className="flex items-center gap-2">
                                    <button
                                      ...
                                      className={cn(
                                        ...
                                        (nontunaiSales[item.id] || 0) > 0
                                          ? "bg-rose-50 hover:bg-rose-100 text-rose-600 border-rose-200 dark:bg-rose-950/20 dark:text-rose-450 dark:border-rose-900"
                                          : "bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-300 dark:text-slate-650 cursor-not-allowed pointer-events-none"
                                      )}
                                    >
                                      - 1
                                    </button>
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => updateNontunaiSale(item.id, 1)}
                                      className="flex-1 py-2 px-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl transition-all active:scale-95 cursor-pointer flex items-center justify-center gap-2 font-black font-mono text-[9px] uppercase tracking-wider shadow-sm shadow-indigo-500/10 border border-indigo-700 dark:bg-indigo-720"
                                    >
                                ...
                                <div className="flex gap-2 pt-2 border-t border-slate-100 dark:border-slate-800 mt-1 select-none font-sans">
                                  <button
                                    onClick={() => handleStartEdit(item)}
                                    className="flex-1 font-black text-[9px] uppercase tracking-wider py-2.5 rounded-xl flex items-center justify-center gap-1.5 cursor-pointer transition-all border border-indigo-200 dark:border-indigo-800 bg-indigo-50 hover:bg-indigo-600 dark:bg-indigo-950/20 text-indigo-700 hover:text-white dark:text-indigo-400 dark:hover:bg-indigo-700"
                                  >
                                    <Edit3 size={11} className="stroke-[2.5]" />
                                    <span>Klik Edit</span>
                                  </button>
                                  ...
                                  <button
                                    ...
                                    className="px-3.5 bg-rose-50 hover:bg-rose-100 text-rose-600 font-black text-[9px] uppercase tracking-wider py-2.5 rounded-xl flex items-center justify-center gap-1.5 cursor-pointer transition-all border border-rose-200 dark:border-rose-900 dark:bg-rose-950/10 dark:text-rose-450"
                                  >
                                    <Trash2 size={12} />
                                    <span>Hapus</span>
                                  </button>
                  ...
                  {/* ====== VIEW 2: MODE EXCEL LENGKAP ====== */}
                  {masterMode === 'excel' && (
                    <div className="bg-white dark:bg-slate-800 border border-slate-205 rounded-2xl overflow-hidden shadow-xs p-3">
                      <div className="overflow-x-auto">
                        <table className="w-full border-collapse border border-slate-150 text-left text-[11px] select-none">
                          <thead>
                            <tr className="bg-slate-150 text-slate-550 font-black tracking-wider uppercase text-[8.5px] border-b border-slate-200 dark:border-slate-700 select-none whitespace-nowrap">
                              <th className="p-3.5 border-r border-slate-205 text-center whitespace-nowrap w-[1%]">No</th>
                              <th className="p-3.5 border-r border-slate-205 whitespace-nowrap w-auto">Nama Voucher</th>
                              <th className="p-3.5 border-r border-slate-205 text-center whitespace-nowrap w-[1%]">Provider</th>
                              <th className="p-3.5 border-r border-slate-205 text-right whitespace-nowrap w-[1%]">Modal</th>
                              <th className="p-3.5 border-r border-slate-205 text-right whitespace-nowrap w-[1%]">Jual</th>
                              <th className="p-3.5 border-r border-slate-205 text-center whitespace-nowrap w-[1%]">Stok</th>
                              <th className="p-3.5 text-center whitespace-nowrap w-[1%]">Aksi (CLOUD)</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-slate-100 bg-white dark:bg-slate-800 whitespace-nowrap">
                            {filteredItems.map((item, idx) => {
                              ...
                              return (
                                <tr key={item.id} className="hover:bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 transition-colors">
                                  {/* No index */}
                                  <td className="p-3 border-r border-slate-150 text-center font-mono text-slate-450 font-bold select-none w-[1%] whitespace-nowrap">{idx + 1}</td>

                                  {/* Nama Voucher */}
                                  <td className="p-3 border-r border-slate-150 w-auto whitespace-nowrap">
                                    {isRowEditing ? (
                                      <input 
                                        type="text"
                                        value={localEdit.name ?? item.name}
                                      ...
                                      <span className="font-extrabold text-slate-850 text-[11.5px] uppercase tracking-wide leading-relaxed block select-text whitespace-nowrap">
                                        {item.name}
                                      </span>
                                    )}
                                  </td>
                                  ...
                                  <td className="p-3 border-r border-slate-150 w-[1%] whitespace-nowrap">
                                    {isRowEditing ? (
                                      <select
                                        value={localEdit.brand ?? item.brand}
                                        onChange={e => handleUpdateEditState(item.id, 'brand', e.target.value)}
                                      ...
                                      <span className={cn("inline-block text-[9px] font-black tracking-widest px-2.5 py-1 border rounded uppercase leading-none mx-auto block w-fit", getBadgeStyles(item.brand))}>
                                        {item.brand}
                                      </span>
                                    )}
                                  </td>
                                  ...
                                  {/* Harga Modal */}
                                  <td className="p-3 border-r border-slate-150 text-right w-[1%] whitespace-nowrap">
                                    {isRowEditing ? (
                                      <div className="relative">
                                  ...
                                  {/* Stock level */}
                                  <td className="p-3 border-r border-slate-150 text-center w-[1%] whitespace-nowrap">
                                    {isRowEditing ? (
                                      <input
                                        type="number"
                                        min="0"
                                  ...
                                  <td className="p-3 text-center w-[1%] whitespace-nowrap">
                                    <div className="flex items-center justify-center gap-1.5 select-none h-full whitespace-nowrap">
                                      {isRowEditing ? (
                                        <>
                                          <button
                                            onClick={() => handleSaveEditItem(item.id)}
...
export default function VouchersWrapper() {
  const { storeName, cashierName } = useStore();
  ...
  const onConfirm = (title: string, message: string, cb: () => void) => {
  ...
  return (
    <div className="h-[calc(100vh-64px)] overflow-y-auto">
      <VoucherView 
        ...
        showToast={showToast}
        onConfirm={onConfirm}
        activeStoreId={storeName || 'default_store'}
        kasirRole="owner"
        kasirName={cashierName || 'KASIR'}
        currentUsername={cashierName || 'KASIR'}
      />
    </div>
  );
}
